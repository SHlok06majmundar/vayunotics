...
export default function DivyalinkInterface() {
    ...
    const fetchFlightModes = (retryCount = 0) => {
      ...
      fetch("https://vayunotics-1.onrender.com/get_flight_modes")
        ...
        .catch(e => {
          ...
          if (retryCount < 5) {
            retryTimeout = setTimeout(() => fetchFlightModes(retryCount + 1), 2000);
          }
        })
  ...
  const getModeName = (id) => supportedModes && supportedModes[id] ? supportedModes[id] : id;
  // Helper: get mode id by name
  const getModeIdByName = (name) => {
    if (!supportedModes) return null;
    return Object.keys(supportedModes).find(key => supportedModes[key] === name);
  };
  ...
  useEffect(() => {
    ...
    const fetchFrameType = (retryCount = 0) => {
      fetch("https://vayunotics-1.onrender.com/get_parameter/FRAME_TYPE")
        ...
        .then(data => {
          if (typeof data.value !== "undefined") setFrameType(Number(data.value));
        })
        .catch(e => {
          if (retryCount < 5) {
            retryTimeoutType = setTimeout(() => fetchFrameType(retryCount + 1), 2000);
          } else {
    ...
    const fetchFrameClass = (retryCount = 0) => {
      fetch("https://vayunotics-1.onrender.com/get_parameter/FRAME_CLASS")
        ...
        .then(data => {
          if (typeof data.value !== "undefined") setFrameClass(Number(data.value));
        })
        .catch(e => {
          if (retryCount < 5) {
            retryTimeoutClass = setTimeout(() => fetchFrameClass(retryCount + 1), 2000);
          } else {
    ...
    const fetchFlightModes = (retryCount = 0) => {
      ...
      fetch("https://vayunotics-1.onrender.com/get_flight_modes")
        ...
        .catch(e => {
          if (retryCount < 5) {
            retryTimeoutModes = setTimeout(() => fetchFlightModes(retryCount + 1), 2000);
          } else {
            toast.error("Failed to fetch flight modes: " + e.message);
          }
        })
  ...
  const renderFlightModesSection = () => {
    ...
    if (!flightModes || !supportedModes || Object.keys(supportedModes).length === 0) {
      return <div style={{ color: 'red', margin: 16 }}>No flight modes available. Please check connection and configuration.</div>;
    }
    ...
    return (
      <div className="mb-4 rounded-xl bg-gray-50 border border-gray-200 shadow-sm p-4">
        <h4 className="text-base font-semibold text-gray-700 mb-3">Flight Modes (1-6)</h4>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-gray-100">
                <th className="py-2 px-3 text-left font-medium text-gray-600 rounded-tl-lg">Slot</th>
                <th className="py-2 px-3 text-left font-medium text-gray-600">Assigned Mode</th>
                <th className="py-2 px-3 text-center font-medium text-gray-600 rounded-tr-lg">Active</th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: 6 }, (_, i) => {
                ...
                if (assignedId !== undefined && assignedId !== null) assignedId = String(assignedId);
                ...
                return (
                  <tr key={slot} className="border-b last:border-b-0">
                    <td className="py-2 px-3 font-semibold text-gray-700">{slot.replace('FLTMODE', 'Mode ')}</td>
                    <td className="py-2 px-3">
                      <select
                        value={assignedId ?? ''}
                        onChange={e => setFlightModes(fm => ({ ...fm, [slot]: e.target.value }))}
                        ...
                        <div className="relative h-full flex items-center justify-center">
                          <div className="relative w-80 h-64">
                            ...
                            <div className="absolute inset-2 rounded-lg overflow-hidden">
                              ...
                              <div className="absolute inset-0 bg-gradient-to-b from-[#60A5FA] to-[#B3E0FF]" />
                              <div
                                className="absolute left-1/2"
                                style={{
                                  transform: `translateX(-50%) rotate(${telemetry.heading || 0}deg)`,
                                  transition: "transform 0.2s linear",
                                }}
                              >
                                <div className="w-0.5 h-8 bg-blue-600 rounded-full" />
                              ...
                              <div className="absolute w-full text-center top-8 text-sm font-mono text-gray-800">
                                {typeof telemetry.heading === "number"
                                  ? `${telemetry.heading.toFixed(0)}° (${getCompassDirection(telemetry.heading)})`
                                  : "Unknown"}
                              </div>
                              ...
                              <div
                                ...
                                style={{
                                  height: '60%',
                                  top: `calc(50% + ${(telemetry.pitch || 0) * -1.5}px)`,
                                  transform: `rotate(${-telemetry.roll || 0}deg)`,
                                  background: 'linear-gradient(to bottom, #4CAF50 60%, #388E3C 100%)',
                            ...
                            <div className="absolute inset-2 pointer-events-none select-none">
                              ...
                              <div
                                ...
                                style={{
                                  transform: `translateY(${(telemetry.pitch || 0) * -1.5}px)`
                                }}
                              />
                              ...
                              >
                                <svg width="288" height="60" viewBox="0 0 288 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  {/* Arc */}
                                  <path d="M24,56 A120,120 0 0,1 264,56" stroke="#fff" strokeWidth="2" fill="none" />
                                  {/* Markers every 15 deg from -45 to +45 */}
                                  {Array.from({ length: 7 }, (_, i) => {
                                    const rollVal = (i - 3) * 15;
                                    const angle = (rollVal + 90) * (Math.PI / 180);
                                    ...
                                    <div className="space-y-6 flex-1 overflow-auto">
                                      ...
                                      <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-md hover:shadow-xl transition-all duration-300">
                                        ...
                                        <div className="text-sm font-semibold text-gray-800 mb-4 flex items-center justify-between">
                                          <div className="flex items-center space-x-2">
                                            <Compass size={16} className="text-blue-600" />
                                            <span>SIMPLE ACCELEROMETER CALIBRATION</span>
                                          </div>
                                          {!accelSimpleCalibrating && !accelSimpleCalDone && (
                                            <button
                                          ...
                                          {accelSimpleCalDone && !accelSimpleCalibrating && (
                                            <span className="px-4 py-1.5 bg-green-600 text-white text-xs rounded-lg">Done</span>
                                          )}
                                        </div>
                                        <div className="text-xs text-gray-600 mb-3">
                                          {accelSimpleCalibrating ? (
                                            <div>Simple accelerometer calibration in progress. Please keep the vehicle level and still.</div>
                                          ) : accelSimpleCalDone ? (
                                            <div className="text-green-600 font-semibold">Completed</div>
                                          ) : (
                                            <>
                                              <div>Use this to perform a simple accelerometer calibration. Place the vehicle on a level surface and click Start Simple Calibration.</div>
                                            </>
                                          )}
                                          {accelSimpleCalStatus && <div className="text-blue-600 font-semibold mt-2">{accelSimpleCalStatus}</div>}
                                          {accelSimpleCalError && <div className="text-red-600 font-semibold mt-2">{accelSimpleCalError}</div>}
                                        </div>
                                        {/* </div> */}
                                        <div className="text-sm font-semibold text-gray-800 mb-4 flex items-center justify-between">
                                          <div className="flex items-center space-x-2">
                                            <Compass size={16} className="text-blue-600" />
                                            <span className="text-xs">ACCELEROMETER OFFSETS CALIBRATION</span>
                                          </div>
                                          {!accelOffsetsCalibrating && !accelOffsetsCalDone && (
                                            <button
                                          ...
                                          {accelOffsetsCalDone && !accelOffsetsCalibrating && (
                                            <span className="px-4 py-1.5 bg-green-600 text-white text-xs rounded-lg">Done</span>
                                          )}
                                        </div>
                                        <div className="text-xs text-gray-600 mb-3">
                                          {accelOffsetsCalibrating ? (
                                            <div>Board level calibration in progress. Please keep the vehicle level and still.</div>
                                          ) : accelOffsetsCalDone ? (
                                            <div className="text-green-600 font-semibold">Completed</div>
                                          ) : (
                                            <>
                                              <div>Use this to calibrate the accelerometer offsets (board level). Place the vehicle on a level surface and click Calibrate Offsets.</div>
                                            </>
                                          )}
                                          {accelOffsetsCalStatus && <div className="text-blue-600 font-semibold mt-2">{accelOffsetsCalStatus}</div>}
                                          {accelOffsetsCalError && <div className="text-red-600 font-semibold mt-2">{accelOffsetsCalError}</div>}
                                        </div>
                                      </div>
                                        ...
                                        {telemetry.compass_calibration.length > 0 && (
                                          <>
                                            {telemetry.compass_calibration.map((compass, index) => {
                                              ...
                                              const displayPct = compass.cal_status === 3 ? 100 : compass.completion_pct;
                                              return (
  ...
  `}</style>
            ...
            <div className="absolute top-16 right-4 bg-white/90 p-4 rounded-xl shadow-xl backdrop-blur-md border border-gray-100 z-10 transition-all duration-300 hover:shadow-2xl">
