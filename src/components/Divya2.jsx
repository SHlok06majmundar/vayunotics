...
export default function DivyalinkInterface({ logs, telemetry, land, takeoff, changeAltitude }) {
  ...
  useEffect(() => {
    if (connected) {
      const telemetryTimer = setInterval(() => {
        ...
        // Update drone speed
        setDroneSpeed((prev) => Math.max(0, prev + (Math.random() - 0.5) * 0.5))
      }, 2000)
  ...
  const handleTakeoff = () => {
    ...
    // Add log entry
    defaultLogs.unshift("Takeoff initiated at " + new Date().toLocaleTimeString())
  }
  ...
  const handleLand = () => {
    ...
    // Add log entry
    defaultLogs.unshift("Landing sequence initiated at " + new Date().toLocaleTimeString())
  }
  ...
  const handleMissionStart = () => {
    ...
    // Add log entry
    defaultLogs.unshift("Mission started at " + new Date().toLocaleTimeString())
  }
  ...
  const handleReturnHome = () => {
    ...
    // Add log entry
    defaultLogs.unshift("Return to home activated at " + new Date().toLocaleTimeString())
  }
  ...
  const handleChangeAltitude = () => {
    ...
    // Add log entry
    defaultLogs.unshift("Altitude change requested at " + new Date().toLocaleTimeString())
  }
  ...
  const handleMapClick = (event) => {
    ...
    const newWaypoint = { lat, lng, alt: altitude, name }
    setWaypoints([...waypoints, newWaypoint])

    // Add log entry
    defaultLogs.unshift(`New waypoint added at ${lat.toFixed(6)}, ${lng.toFixed(6)}`)
  }
  ...
  const handleFlyHere = (waypoint) => {
    setStatusMessage(`Flying to ${waypoint.name} at coordinates ${waypoint.lat.toFixed(6)}, ${waypoint.lng.toFixed(6)}`)
    setTimeout(() => setStatusMessage(""), 3000)

    // Add log entry
    defaultLogs.unshift(`Flying to ${waypoint.name} at ${waypoint.lat.toFixed(6)}, ${waypoint.lng.toFixed(6)}`)
  }
  ...
  return (
    <div className={`fixed inset-0 flex flex-col ${darkMode ? "dark" : ""}`}>
      ...
      <div className="flex flex-col h-full bg-white text-slate-900 dark:bg-slate-900 dark:text-white select-none font-['Nasalization'] transition-colors duration-300">
        ...
        <div className="flex justify-between items-center bg-white dark:bg-gradient-to-r dark:from-slate-900/90 dark:via-gray-900/80 dark:to-slate-800/90 p-4 border-b border-slate-200 dark:border-slate-700/30 shadow-md backdrop-blur-md z-10">
          ...
          <div className="flex items-center space-x-2 md:space-x-6">
              ...
              >
                <div
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    connected ? "bg-emerald-400 dark:animate-pulse-fast dark:shadow-glow" : "bg-red-400 dark:shadow-md"
                  }`}
                ></div>
            ...
            >
              ...
              ></div>
              <span className="text-xs font-semibold">{connected ? "ON" : "OFF"}</span>
            </button>
          </div>
        </div>
        ...
        <div
          className={`fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-slate-900 shadow-xl transform transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:hidden`}
        >
              ...
              <div className="flex items-center justify-between p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full ${connected ? "bg-emerald-400 animate-pulse" : "bg-red-400"}`}
                  ></div>
                  <span className="text-sm font-semibold">{connected ? "CONNECTED" : "CONNECT"}</span>
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div
            className={`w-full md:w-1/3 border-r border-slate-200 dark:border-slate-800 flex flex-col bg-white dark:bg-gradient-to-b dark:from-slate-900/95 dark:via-gray-900/90 dark:to-slate-800/95 ${activeTab === "DATA" ? "block" : "hidden md:flex"}`}
          >
            {/* Attitude Indicator */}
                    ...
                    <div className="flex justify-between items-center px-4">
                      <div
                        className={`text-sm font-semibold tracking-wide ${
                          isArmed ? "text-emerald-500 dark:text-emerald-300" : "text-red-500 dark:text-red-300"
                        } flex items-center space-x-2`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${
                            isArmed ? "bg-emerald-500 animate-pulse" : "bg-red-500 animate-pulse"
                          }`}
                        />
              ...
              {activeTabButton === "Actions" ? (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    />
                    <ActionButton
                      icon={<Plane className="transform rotate-180 hover:scale-110 transition-transform" size={24} />}
                      label="LAND"
                      onClick={land}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <ActionButton
                      icon={<Play className="transform hover:scale-110 transition-transform" size={24} />}
                      label="MISSION START"
                ...
                <div className="flex flex-col h-full">
                  <div className="bg-slate-100 dark:bg-slate-800/90 p-3 text-center font-semibold text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700/50 backdrop-blur-md flex items-center justify-center space-x-2 mb-4 rounded-lg">
                    <BarChart2 size={16} className="text-cyan-500 dark:text-cyan-400" />
                    <span>SYSTEM MESSAGES</span>
                  </div>
                  <div className="space-y-3 flex-1 overflow-auto">
                    {(logs.length > 0 ? logs : defaultLogs).map((log, index) => (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        <div className="text-sm text-slate-700 dark:text-slate-200 mt-1">{log}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col h-full">
                  <div className="bg-slate-100 dark:bg-slate-800/90 p-3 text-center font-semibold text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700/50 backdrop-blur-md flex items-center justify-center space-x-2 mb-4 rounded-lg">
                    <Layers size={16} className="text-cyan-500 dark:text-cyan-400" />
                          ...
                          waypoints.map((wp, i) => (
                            ...
                            >
                              <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm text-slate-700 dark:text-slate-200">{wp.name}</span>
                              </div>
                              <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="p-3 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/40">
                          <div className="text-xs text-slate-500 dark:text-slate-400">Altitude</div>
                          <div className="text-sm text-slate-700 dark:text-slate-200 flex items-center space-x-1">
                            <ArrowUp size={12} className="text-cyan-500 dark:text-cyan-400" />
                            <span>50 meters</span>
                          </div>
                        </div>
                        <div className="p-3 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/40">
                          ...
                          <div className="text-sm text-slate-700 dark:text-slate-200 flex items-center space-x-1">
                            <Zap size={12} className="text-cyan-500 dark:text-cyan-400" />
                            <span>{droneSpeed.toFixed(1)} m/s</span>
                          </div>
                        </div>
                        <div className="p-3 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/40">
                          <div className="text-xs text-slate-500 dark:text-slate-400">Mission Type</div>
                          <div className="text-sm text-slate-700 dark:text-slate-200 flex items-center space-x-1">
                        ...
                        <div className="p-3 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/40">
                          ...
                          <div className="text-sm text-slate-700 dark:text-slate-200 flex items-center space-x-1">
                            <Clock size={12} className="text-cyan-500 dark:text-cyan-400" />
                ...
                <div className="flex flex-col h-full">
                  <div className="bg-slate-100 dark:bg-slate-800/90 p-3 text-center font-semibold text-slate-700 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700/50 backdrop-blur-md flex items-center justify-center space-x-2 mb-4 rounded-lg">
                    <AlertTriangle size={16} className="text-cyan-500 dark:text-cyan-400" />
                    <span>SYSTEM STATUS</span>
                  </div>
                  <div className="space-y-4 flex-1 overflow-auto">
                    <div className="p-4 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/40">
                      <div className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2 flex items-center space-x-2">
                        <Cpu size={14} className="text-cyan-500 dark:text-cyan-400" />
                        <span>HARDWARE</span>
                      </div>
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-slate-700/30 rounded-lg">
                          ...
                          <span className="text-emerald-500 dark:text-emerald-400">12GB free</span>
                        </div>
                        <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-slate-700/30 rounded-lg">
                          <span className="text-slate-700 dark:text-slate-300 flex items-center space-x-1">
                            <Zap size={12} />
                          ...
                          <span className="text-amber-500 dark:text-amber-400">42°C</span>
                        </div>
                      </div>
                    </div>
                    <div className="p-4 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/40">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-slate-700/30 rounded-lg">
                          ...
                          <span className="text-emerald-500 dark:text-emerald-400">8 satellites</span>
                        </div>
                        <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-slate-700/30 rounded-lg">
                          <span className="text-slate-700 dark:text-slate-300 flex items-center space-x-1">
                            <Compass size={12} />
                          ...
                          <span className="text-emerald-500 dark:text-emerald-400">Calibrated</span>
                        </div>
                        <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-slate-700/30 rounded-lg">
                          <span className="text-slate-700 dark:text-slate-300 flex items-center space-x-1">
                            <ArrowUp size={12} />
                        ...
                        <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-slate-700/30 rounded-lg">
                          ...
                          <span className="text-amber-500 dark:text-amber-400">Needs Calibration</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/40">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-slate-700/30 rounded-lg">
                          ...
                          <span className={`${getBatteryColor()} font-semibold`}>{batteryLevel.toFixed(1)}%</span>
                        </div>
                        <div className="w-full h-3 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${
                              batteryLevel > 60 ? "bg-emerald-500" : batteryLevel > 20 ? "bg-amber-500" : "bg-red-500"
                            }`}
                ...
                <div className="space-y-4">
                  ...
                  <div className="mt-6 flex justify-center">
                    <button
                      ...
                      onClick={toggleArmed}
                    >
                      <Power size={18} className={isArmed ? "text-red-200" : "text-emerald-200"} />
          ...
          {/* Right Panel - Map */}
          <div
            className={`w-full md:w-2/3 bg-white dark:bg-slate-900 flex flex-col relative ${activeTab === "DATA" ? "hidden md:flex" : "flex"}`}
          >
            {isLoaded ? (
              <GoogleMap
              ...
              >
                ...
                {selectedWaypoint && (
                  <InfoWindow position={selectedWaypoint} onCloseClick={() => setSelectedWaypoint(null)}>
                    <div className="bg-white p-4 rounded-lg shadow-lg max-w-[300px]">
                      <h3 className="font-bold text-lg mb-2">🎯 Waypoint {waypoints.indexOf(selectedWaypoint) + 1}</h3>
                      <div className="space-y-2">
                        <p className="text-sm">
                          <span className="font-semibold">📍 Name:</span> {selectedWaypoint.name || "Unnamed Waypoint"}
                        </p>
                        ...
                        <p className="text-sm">
                          ...
                          {selectedWaypoint.alt !== undefined ? `${selectedWaypoint.alt}m` : "N/A"}
                        </p>
                        <button
                ...
                {/* Drone Marker */}
                <Marker
                  position={{ lat: droneData.latitude, lng: droneData.longitude }}
                  icon={getDroneIcon()}
              ...
              <div className="flex flex-col space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className={`text-sm font-semibold ${getBatteryColor()}`}>{batteryLevel.toFixed(1)}%</span>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Gauge className="text-blue-500 dark:text-blue-400" size={16} />
                  <div className="flex flex-col">
                ...
                <div className="flex items-center space-x-2">
                  <ArrowUp className="text-cyan-500 dark:text-cyan-400" size={16} />
                  <div className="flex flex-col">
                    <span className="text-xs text-slate-500 dark:text-slate-400">Altitude</span>
                ...
                <div className="space-y-1">
                  {waypoints.map((wp, index) => (
                    ...
                    >
                      ...
                      <button onClick={() => removeWaypoint(index)} className="bg-red-500 text-white px-1 rounded">
                        ✕
                      </button>
                    </div>
                  ))}
...
function TelemetryItem({ label, value, color, icon, darkMode }) {
  ...
  return (
    <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
      ...
      <div className={`text-lg font-semibold ${color} flex items-center space-x-2`}>
        {icon}
        <span>{value}</span>
      </div>
    </div>
